...
OUT_DIR="$REPORT_ROOT/${TIMESTAMP}-local-embeddeddb-write-with-metrics"
SAMPLE_SECONDS="${SILVERLINK_LOCAL_METRICS_SAMPLE_SECONDS:-20}"

mkdir -p "$OUT_DIR"

echo "# 本机嵌入式数据库写压联合采样" >"$OUT_DIR/summary.md"
echo >>"$OUT_DIR/summary.md"
echo "- 时间：$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$OUT_DIR/summary.md"
echo "- 模式：本机 H2 嵌入式数据库 + 真实 JDBC 写链路 + 本机资源采样" >>"$OUT_DIR/summary.md"
...
(
  cd "$BACKEND_DIR"
  ./mvnw -DargLine= -Dtest=SmsRelayEmbeddedDbWritePerfTest surefire:test
...
LATEST_MD="$(find "$REPORT_ROOT" -maxdepth 1 -type f -name '*local-embeddeddb-write-pressure.md' | sort | tail -n 1)"
LATEST_JSON="$(find "$REPORT_ROOT" -maxdepth 1 -type f -name '*local-embeddeddb-write-pressure.json' | sort | tail -n 1)"
