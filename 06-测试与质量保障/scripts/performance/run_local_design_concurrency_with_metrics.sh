...
OUT_DIR="$REPORT_ROOT/${TIMESTAMP}-local-design-concurrency-with-metrics"
SAMPLE_SECONDS="${SILVERLINK_LOCAL_METRICS_SAMPLE_SECONDS:-20}"

mkdir -p "$OUT_DIR"

echo "# 本机联合压测与资源采样" >"$OUT_DIR/summary.md"
echo >>"$OUT_DIR/summary.md"
echo "- 时间：$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$OUT_DIR/summary.md"
echo "- 模式：本机 JVM 并发设计探针 + 本机资源采样" >>"$OUT_DIR/summary.md"
echo "- 目标：观察设计缺陷触发时的 JVM / 进程 / 内存指标，不把本机绝对耗时当作通用性能结论" >>"$OUT_DIR/summary.md"
...
LATEST_PROBE_MD="$(find "$REPORT_ROOT" -maxdepth 1 -type f -name '*local-scan-write-design-probe.md' | sort | tail -n 1)"
LATEST_PROBE_JSON="$(find "$REPORT_ROOT" -maxdepth 1 -type f -name '*local-scan-write-design-probe.json' | sort | tail -n 1)"

{
  ...
  echo "- 关联探针报告：\`$(basename "$LATEST_PROBE_MD")\`"
  echo "- 关联探针 JSON：\`$(basename "$LATEST_PROBE_JSON")\`"
} >>"$OUT_DIR/summary.md"
