...
- 三个前端页面是否加载过慢
- 微信小程序 Taro 构建、分包体积和微信 CI 包体是否异常
...
- Taro build：`npm run test:build-performance` 通过，完整门禁耗时 13703 / 40000 ms，263 modules transformed。
- dist 文件数：73。
- dist 总大小：529578 bytes。
- 本地性能预算门禁：`npm run test:performance-budget` 通过。
  - `dist` 总包：529578 / 2097152 bytes。
  - 扫码分包：55692 / 512000 bytes。
  - 工作台分包：78443 / 716800 bytes。
  - gzip 基线：146944 bytes。
- 微信 CI 包体：`/subpackages/scan/` 60271 bytes，`/subpackages/workbench/` 86689 bytes，`__APP__` 428917 bytes，`__FULL__` 575877 bytes。
- 报告路径：`06-测试与质量保障/reports/regression/20260606-190933-weapp-local-comprehensive/performance.json`。
...
- 志愿者端登录页
- 志愿者端老人列表页
- 微信小程序首页、扫码落地页、扫码验证页、工作台老人列表页、二维码/名牌页（需 DevTools 或真机运行时环境；不属于当前代码层性能门禁）
